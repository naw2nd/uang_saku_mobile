...
class UpdatePengajuanReimburse extends StatefulWidget {
  final pengajuan;
  UpdatePengajuanReimburse({this.pengajuan});
  @override
  _UpdatePengajuanReimburseState createState() => _UpdatePengajuanReimburseState();
}

class _UpdatePengajuanReimburseState extends State<UpdatePengajuanReimburse> {
  var _colorTheme = Color(0xFF3AE3CE);
  ...
  @override
  void initState() {
    BlocProvider.of<ReimburseBloc>(context).add(GetFormAttributeReimburse());
    ...
    return MaterialApp(
      home: Scaffold(
        ...
        body: SingleChildScrollView(
            child: Form(
          key: _formKey,
          child: BlocConsumer<ReimburseBloc, BaseState>(
              listener: (context, state) {
            if (state is SuccesState) {
              Scaffold.of(context).showSnackBar(SnackBar(
                  content:
                      Text("Reimburse berhasil dikirimkan")));
              Timer(
                  Duration(seconds: 2),
            ...
            if (state is FormAttributeStateReimburse) {
